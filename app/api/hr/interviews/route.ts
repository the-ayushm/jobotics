...
export async function POST(req: NextRequest) {
    try {
        ...
        let meetLink = '';
        if (process.env.GOOGLE_CALENDAR_CLIENT_ID && process.env.GOOGLE_REFRESH_TOKEN) {
            try {
                ...
                console.error("Error creating Google Meet event:", calError.message);
            }
        }
        ...
        **Interview Details:**
        ...
        - **Interviewer(s):** HR Team
        - **Platform:** Google Meet
        - **Join Link:** ${meetLink || 'Google Meet link will be shared by HR prior to the call.'}
        ...
        if (process.env.GOOGLE_CALENDAR_EMAIL && process.env.GOOGLE_REFRESH_TOKEN) {
            try {
